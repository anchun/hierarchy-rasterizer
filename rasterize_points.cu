	...
	const float big_limit)
{
  ...
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth, out_semantic);
}
	...
	const bool debug)
{
  ...
  torch::Tensor dL_dsemantic = torch::zeros({fullP, S}, means3D.options());
